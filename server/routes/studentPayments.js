...
async function getStudentPaymentSummary(studentId) {
  const row = await db.get(
    `SELECT
       COALESCE(SUM(COALESCE(course_fee, 0)), 0) as totalFees,
...
// Filter options for student payment list (cohorts, courses)
...
// Combined student detail (payments + courses + transactions in one request)
...
router.get('/students', authenticateToken, requireStudentPaymentAccess(), async (req, res) => {
  try {
    ...
    let query = `
      SELECT s.*, u.name, u.email, u.phone, u.profile_image,
             ch.name as cohort_name, ch.code as cohort_code,
             COALESCE(ps.totalFees, 0) as totalFees,
             COALESCE(ps.totalPaid, 0) as totalPaid,
             COALESCE(ps.totalBalance, 0) as totalBalance,
      ...
      LEFT JOIN (
        SELECT student_id,
               SUM(COALESCE(course_fee, 0)) as totalFees,
